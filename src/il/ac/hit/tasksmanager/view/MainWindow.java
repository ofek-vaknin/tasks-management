 ...
 * on the EDT.
 * Acts as an Observer of the ViewModel by implementing {@link ViewModelObserver}.
 */
public class MainWindow extends JFrame implements ViewModelObserver {
    ...
    public MainWindow(TasksListViewModel viewModel) {
        ...
        /* Observer registration: subscribe this view to ViewModel updates */
     ...
     */
    private void onEdit() {
        /* Edit the first selected task: read selection, show dialog, forward update to ViewModel. */
        List<Integer> ids = tablePanel.getSelectedTaskIds();
        if (ids.isEmpty()) {
    ...
    private void onDelete() {
        /* Delete all selected tasks after confirmation by delegating to the ViewModel. */
        List<Integer> ids = tablePanel.getSelectedTaskIds();
        if (ids.isEmpty()) {
    ...
    @Override
    public void update() {
        /* Observer update entry point from ViewModel */
        refreshTable();
    }
}
